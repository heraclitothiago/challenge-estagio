...
class Pager
{
    ...
    public function render(string $cssClass = null, bool $fixedFirstAndLastPage = true): ?string
    {
        ...
        if ($this->rows > $this->limit):
            ...
            $paginator .= "<span class=\"{$this->class}page-link {$this->class}_active\">{$this->page}</span>";
     ...
     * @return null|string
     */
    private function beforePages(string $class = 'page-item'): ?string
    {
        $before = null;
        for ($iPag = $this->page - $this->range; $iPag <= $this->page - 1; $iPag++):
            if ($iPag >= 1):
                $before .= "<a class='{$this->class}{$class}' title=\"{$this->title} {$iPag}\" href=\"{$this->link}{$iPag}{$this->hash}{$this->params}\">{$iPag}</a>";
                $before .= "</ul>\n";
            endif;
        endfor;
     ...
     * @return string|null
     */
    private function afterPages(string $class = 'page-item'): ?string
    {
        $after = null;
        for ($dPag = $this->page + 1; $dPag <= $this->page + $this->range; $dPag++):
            if ($dPag <= $this->pages):
                $after .= "\n<li class='{$class}'>";
                $after .= "<a class='{$this->class}page-link' title=\"{$this->title} {$dPag}\" href=\"{$this->link}{$dPag}{$this->hash}{$this->params}\">{$dPag}</a>";
                $after .= "</li>";
            endif;
        endfor;
